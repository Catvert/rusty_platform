...
pub fn draw_sprite(ctx: &mut Context, camera: &Camera, spr_image: &SpriteImage, rect: &Rect, mode: &SpriteMode) {
    ...
    match mode {
        ...
        SpriteMode::Repeat(size) => {
            let scale = camera.world_size_to_screen(&Vector2::new(size.x as f64 / image.width() as f64, size.y as f64 / image.height() as f64));
            ...
            for x in 0..rows {
                for y in 0..cols {
                    let pos_in_screen = camera.world_point_to_screen(&Point2::new(rect.pos.x + (x * size.x) as f64, rect.pos.y + (y * size.y) as f64));
